...
abstract class Logic<T> implements AbstractLogic<T> {
  ...
  Reducer<T>? createReducer() {
    final Reducer<T> combineReducer = combineReducers<T>(<Reducer<T>>[
      if (protectedReducer != null) protectedReducer!,
      if (protectedDependenciesReducer != null) protectedDependenciesReducer!,
    ]);

    return protectedFilter != null
        ? helper.filterReducer(combineReducer, protectedFilter!)
        : combineReducer;
  }

  @override
  Object onReducer(Object state, Action action) =>
      cache<Reducer<T>?>('onReducer', createReducer)
