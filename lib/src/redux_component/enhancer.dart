import 'package:fish_redux_ns/src/redux_component/helper.dart';
...
class EnhancerDefault<T> implements Enhancer<T> {
  ...
  ViewBuilder<K>? viewEnhance<K>(
    ...
    final ViewBuilder? viewBuilder = _inverterView<K>(view);
    if (viewBuilder == null) {
      return view;
    }
  ...
  AdapterBuilder<K>? adapterEnhance<K>(
    ...
    final AdapterBuilder? viewBuilder = _inverterAdapter<K>(adapterBuilder);
    if (viewBuilder == null) {
      return adapterBuilder;
    }
    return _adapterEnhancer?.call(logic, store).call(viewBuilder) ??
        adapterBuilder;
  }
  ...
  Effect<K>? effectEnhance<K>(
    ...
    final Effect? updatedEffect = _inverterEffect<K>(effect);
    if (updatedEffect == null) {
      return effect;
    }
    return _effectEnhancer?.call(logic, store).call(updatedEffect) ?? effect;
  }
  ...
  Effect<dynamic>? _inverterEffect<K>(Effect<K>? effect) {
    return effect == null
        ? null
        : (Action action, dynamic ctx) => effect(action, ctx);
  }
